...
class Auth
{
     ...
     */
    public function check($name, $uid, $relation = 'or', $mode = 'url',$custom_id = 0)
    {
        ...
        // 获取用户需要验证的所有有效规则列表
        $rulelist = $this->getRuleList($uid,$custom_id);
     ...
     *              ...)
     */
    public function getGroups($uid,$custom_id = 0)
    {
        static $groups = [];
        if (isset($groups[$uid]))
        {
        ...
        // 执行查询
        $sql = "{$auth_group_access}.uid='{$uid}' and {$auth_group}.status='normal'";
        if($custom_id > 0)
        {
            $sql .= " and {$auth_group_access}.custom_id in ('0','{$custom_id}')";
        }
     ...
     * @return array
     */
    public function getRuleList($uid,$custom_id = 0)
    {
        ...
        // 读取用户规则节点
        $ids = $this->getRuleIds($uid,$custom_id);
        if (empty($ids))
        {
            $_rulelist[$uid] = [];
            return [];
    ...
    public function getRuleIds($uid,$custom_id = 0)
    {
        //读取用户所属用户组
        $groups = $this->getGroups($uid,$custom_id);
